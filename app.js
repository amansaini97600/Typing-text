...
const correctWord = document.querySelector('#correct-words');
const wrongWord = document.querySelector('#incorrect-words');
const highestRecordEle = document.querySelector('.highest-recod').innerText = `Highest Rocord: ${localStorage.getItem('WPM')}wpm`;
...
// home and about color set

home.addEventListener('click', () => {
  limitColor(home, about);
});

about.addEventListener('click', () => {
  home.classList.remove('active');
  limitColor(about, home);
})
...
oneMin.addEventListener("click", function () {
  timer = 60;
  factor = 1;
  limitColor(oneMin, twoMin, fiveMin, tenMin);
  time.innerText = `1`;
});
twoMin.addEventListener("click", function () {
  timer = 120;
  factor = 2;
  limitColor(twoMin, oneMin, fiveMin, tenMin);
  time.innerText = `2`;
});
fiveMin.addEventListener("click", function () {
  timer = 300;
  factor = 5;
  limitColor(fiveMin, tenMin, oneMin, twoMin);
  time.innerText = `5`;
});
tenMin.addEventListener("click", function () {
  timer = 600;
  factor = 10;
  limitColor(tenMin, fiveMin, oneMin, twoMin);
  time.innerText = `10`;
});
...
beg.addEventListener("click", function () {
  difficulty = 1;
  displayTest(difficulty);
  limitColor(beg, pro, hindi);
});
pro.addEventListener("click", function () {
  difficulty = 2;
  displayTest(difficulty);
  limitColor(pro, beg, hindi);
});
hindi.addEventListener('click', function() {
  difficulty = 3;
  displayTest(difficulty);
  limitColor(hindi, pro, beg);
});

//set the color of time and difficulty
function limitColor(itema, itemr, color3, color4) {
  itema.classList.add('yellow');
  itemr.classList.remove('yellow');
  color3.classList.remove('yellow');
  color4.classList.remove('yellow');
}
...
function timeStart() {
  ...
  seconds = setInterval(function () {
    if (timer > 0) {
      ...
      var secondsRemaining = (timer % 60);
      time.innerText = minutes + ":" + (secondsRemaining < 10 ? "0" + secondsRemaining : secondsRemaining);
      timer--;
    } else {
      timeOver();
      clearInterval(seconds);
...
function limitVisible() {
  oneMin.style.visibility = 'visible';
  twoMin.style.visibility = 'visible';
  fiveMin.style.visibility = 'visible';
  tenMin.style.visibility = 'visible';
  beg.style.visibility = 'visible';
  pro.style.visibility = 'visible';
}
function limitInvisible() {
  oneMin.style.visibility = 'hidden';
  twoMin.style.visibility = 'hidden';
  fiveMin.style.visibility = 'hidden';
  tenMin.style.visibility = 'hidden';
...
function displayScore() {
  ...
  timeName.innerText = "Acusary";

  let WPM = Math.floor(wordsCorrect / factor);
  cw.innerText = WPM;
  cwName.innerText = "WPM";


  if (WPM > localStorage.getItem('WPM')) {
    highestRecord = WPM;
    localStorage.setItem('WPM', highestRecord);
    document.querySelector('.highest-recod').innerText = `Highest Record : ${localStorage.getItem('WPM')}wpm`;
  }

  document.getElementById('results').style.display = 'flex';
  document.getElementById('totals-words').innerText = wordsSubmitted;
  correctWord.innerText = wordsCorrect;
  wrongWord.innerText = (wordsSubmitted - wordsCorrect);
}
...
function checkWord() {
  ...
  if (wordNo > 40) {
    ...
    colorSpan(nextID, 2);
      // if(wordNo === 41 || (wordNo === 40 && wordEntered !== '')){
...
function randomWords(diff) {
  ...
  var hindi = [
    'अमन ', 'अ ','बी ','क ','क ','ग ', 'फ ', 'प ','र ','ओ ','ल ','र ','स ','श','डी ','टी ','थ ','घ ','ख ','प्रीती ','नैना ','जान ','काजल ','सचिन ','कबिता ','अंकित '
  ];
  ...
  for (var i = 0; i < 40; i++) {
    ...
    let lastWord = selectedWords.push(wordArray[randomNumber] + " ");
  }
